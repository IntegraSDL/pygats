...
def click_text(ctx, text, lang, button='left', skip=0):
    ...
    x, y, width, height, found = find_text_on_screen(ctx, text, lang, skip, True)
    if not found:
...
def find_text_on_screen(ctx, text, lang, skip=0, one_word = False):
    """
    ...
    img = pyautogui.screenshot()
    return find_text(img, text, lang, skip, one_word)
